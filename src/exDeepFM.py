...
class ExtremeDeepFMModel(BaseModel):
    ...
    def _build_graph(self, hparams):
        ...
        self.layer_keeps = tf.placeholder(tf.float32) # what's meaning
        with tf.variable_scope("exDeepFm") as scope:
            with tf.variable_scope("embedding", initializer=self.initializer) as escope:
                self.embedding = tf.get_variable(name='embedding_layer',
                                                 ...
                                                 dtype=tf.float32) # feature_size, em_size
    ...
    def _build_embedding(self, hparams):
        fm_sparse_index = tf.SparseTensor(self.iterator.dnn_feat_indices, # 
                                          ...
                                          self.iterator.dnn_feat_shape) # num_instance * field_num, maximal appearing count amoung fields
        ...
        w_fm_nn_input_orgin = tf.nn.embedding_lookup_sparse(self.embedding, # feats_num, em_size
                                                            fm_sparse_index,
                                                            fm_sparse_weight,
                                                            combiner="sum")
        
        embedding = tf.reshape(w_fm_nn_input_orgin, [-1, hparams.dim * hparams.FIELD_COUNT]) # N, em_size * FIELD_COUNT
    ...
    def _build_extreme_FM(self, hparams, nn_input, res=False, direct=False, bias=False, reduce_D=False, f_dim=2):
        ...
        field_num = hparams.FIELD_COUNT
                             # N, em_size * field_num
        nn_input = tf.reshape(nn_input, shape=[-1, int(field_num), hparams.dim]) # N, filed_num, em_size
        field_nums.append(int(field_num))
        hidden_nn_layers.append(nn_input)
        final_result = []
        split_tensor0 = tf.split(hidden_nn_layers[0], hparams.dim * [1], 2) #em_size, N, filed_num, 1
        with tf.variable_scope("exfm_part", initializer=self.initializer) as scope:
            for idx, layer_size in enumerate(hparams.cross_layer_sizes):
                split_tensor = tf.split(hidden_nn_layers[-1], hparams.dim * [1], 2)
                dot_result_m = tf.matmul(split_tensor0, split_tensor, transpose_b=True) # (em_size, N, field_num, 1) x (em_size, N, H_k,  1)
                dot_result_o = tf.reshape(dot_result_m, shape=[hparams.dim, -1, field_nums[0]*field_nums[-1]]) # em_size, N, H_k x m
                dot_result = tf.transpose(dot_result_o, perm=[1, 0, 2]) # N, em_size, H_k x m

                if reduce_D: # reduce dimention of W
                    hparams.logger.info("reduce_D")
                    filters0 = tf.get_variable("f0_" + str(idx),
                                               shape=[1, layer_size, field_nums[0], f_dim],
                                               dtype=tf.float32) # filter_size, output_channel, input_channel, reduced_dim
                    ...
                    filters = tf.transpose(filters_o, perm=[0, 2, 1]) # filter_size, input_channel, output_channel
                else:
                    filters = tf.get_variable(name="f_"+str(idx),
                                         shape=[1, field_nums[-1]*field_nums[0], layer_size],
                                         dtype=tf.float32) # filter_size, H_k x m, H_(k+1)
                # dot_result = tf.transpose(dot_result, perm=[0, 2, 1])
                curr_out = tf.nn.conv1d(dot_result, filters=filters, stride=1, padding='VALID') # N, em_size, H_(k+1)
                # BIAS ADD
                if bias:
                ...
                curr_out = tf.transpose(curr_out, perm=[0, 2, 1]) # N, H_(k+1), em_size
