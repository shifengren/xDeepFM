...
class FfmCache(BaseCache):
    ...
    def _load_batch_data_from_file(self, file, hparams):
        ...
        impression_id = []
        cnt = 0
        #---------each line has the same format as follows----------
        # 0 1:1806:1 2:3427:1 33:x:0.5%%ID
            ...
            while True:
                ...
                for word in cols[1:]:
                    ...
                    cur_feature_list.append( 
                        [int(tokens[0]) - 1, 
                         int(tokens[1]) - 1, 
                         float(tokens[2])]) # filed, feature, value
    ...
    def _convert_data(self, labels, features, hparams):
        dim = hparams.FEATURE_COUNT # feature count
        FIELD_COUNT = hparams.FIELD_COUNT # field count
        instance_cnt = len(labels) # abservation count
        ...
        dnn_feat_values = []
        dnn_feat_weights = []
        dnn_feat_shape = [instance_cnt * FIELD_COUNT, -1] # ？, filed_count

        for i in range(instance_cnt):
            m = len(features[i])
            dnn_feat_dic = {}
            for j in range(m): # for each filed
        ...
        # instance_cnt * FIELD_COUNT, max(field_count[i])
